...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->tabPage1->Text = L"Ðàññ÷åò âðåìåíè ìåæäó äâóìÿ äàòàìè";
			// 
			// label19
			// 
			...
			this->ClearButton->Text = L"Î÷èñòèòü";
			...
			this->resultButton->Text = L"Ðàññ÷èòàòü";
			...
			this->label6->Text = L"Êîíå÷íàÿ äàòà";
			// 
			// Year2
			// 
			...
			this->label5->Text = L"Íà÷àëüíàÿ äàòà";
			// 
			// Year1
			// 
			...
			this->FormatCB1->Items->AddRange(gcnew cli::array< System::Object^  >(2) { L"ÄÄ/ÌÌ/ÃÃÃÃ", L"ÌÌ/ÄÄ/ÃÃÃÃ" });
			...
			this->label4->Text = L"Âûáåðèòå ôîðìàò";
			// 
			// label3
			// 
			...
			this->label3->Text = L"Ââåäèòå íà÷àëüíóþ è êîíå÷íóþ äàòó è ïîëó÷èòå ðåçóëüòàò!";
			// 
			// label2
			// 
			...
			this->label2->Text = L"Êàëüêóëÿòîð âû÷èñëÿåò êîëè÷åñòâî äíåé ìåæäó äâóìÿ äàòàìè";
			// 
			// label1
			// 
			...
			this->label1->Text = L"Ñêîëüêî âðåìåíè ìåæäó äâóìÿ äàòàìè\?";
			// 
			// tabPage2
			// 
			...
			this->tabPage2->Text = L"Ðàññ÷åò âðåìåíè îò ñåãîäíÿøíåãî äíÿ";
			// 
			// label13
			// 
			...
			this->FormatCB2->Items->AddRange(gcnew cli::array< System::Object^  >(2) { L"ÄÄ/ÌÌ/ÃÃÃÃ", L"ÌÌ/ÄÄ/ÃÃÃÃ" });
			...
			this->label14->Text = L"Âûáåðèòå ôîðìàò";
			// 
			// ClearButton2
			// 
			...
			this->ClearButton2->Text = L"Î÷èñòèòü";
			...
			this->resultButton2->Text = L"Ðàññ÷èòàòü";
			...
			this->label11->Text = L"Ââåäèòå äàòó";
			// 
			// Year3
			// 
			...
			this->label10->Text = L"Ââåäèòå äàòó è ïîëó÷èòå ðåçóëüòàò!";
			// 
			// label9
			// 
			...
			this->label9->Text = L"Êàëüêóëÿòîð âû÷èñëÿåò êîëè÷åñòâî äíåé, ìåñÿöåâ, ëåò äî ëþáîé äàòû";
			// 
			// label8
			// 
			...
			this->label8->Text = L"Ñêîëüêî âðåìåíè äî îïðåäåëåííîé äàòû\?";
			// 
			// ExitButton
			// 
...
#pragma endregion
    ...
    int DaysCount(int day1, int day2, int month1, int month2, int year1, int year2)
    {
		...
		if (year1 != year2)
		{
			...
			switch (month1)//рассмотрение разных случаев (кол-во дней в месяцах разное)
			{
			...
			if (isleap(year2) && month2 > 2) { daysaft += 1; } //если високосный
	...
	int YearCount(int day1, int day2, int month1, int month2, int year1, int year2,int monthcount) //подсчет кол-ва лет - целочисленное деление
	{
		int yearcount;
		if (year1 != year2)
		{
	...
	private: System::Void ClearButton_Click(System::Object^ sender, System::EventArgs^ e) { //очищение полей
	...
	private: System::Void ExitButton_Click(System::Object^ sender, System::EventArgs^ e) {
		Application::Exit(); //кнопка "выход" - завершение работы
	}

    bool isleap(int year) { //високосный ли год
    ...
    private: System::Void ClearButton2_Click(System::Object^ sender, System::EventArgs^ e) { //кнопка "очистить"
	...
	bool CheckInputBoxes2() //ввод только цифр и ограничение на их кол-во - проверка
	   {
	...
	bool CheckBox(int day1, int day2, int month1, int month2, int year1, int year2) //проверка даты (кол-во дней от 1 до 31, месяцев - от 1 до 12)
	{
	...
	{
		int day1, month1, year1, day2, month2, year2;
		if (FormatCB2->SelectedIndex == 0) //если формат ДД.ММ.ГГГГ
		{
		...
		else if (FormatCB2->SelectedIndex == 1) //если формат ММ.ДД.ГГГ
		{
		...
		if (CheckBox(day1, day2, month1, month2, year1, year2)) {
			...
			ResultT1->Text = System::Convert::ToString(daycountt); //вывод в поля результат
    ...
    private: System::Void FormatCB2_SelectedIndexChanged(System::Object^ sender, System::EventArgs^ e) //установка сегодняшней даты после выбора формата даты
    {
